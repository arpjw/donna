"""
New York Attorney General ingester.
Scrapes press releases from https://ag.ny.gov/press-release using httpx + BeautifulSoup.
"""
import logging
...
SOURCE_URL = "https://ag.ny.gov/press-release"
...
DATE_FORMATS = [
    ...
    "%m/%d/%Y",    # 01/15/2026
    "%Y-%m-%d",    # 2026-01-15
    "%B %Y",       # January 2026 (month-only fallback)
]
...
class NewYorkAGIngester(BaseIngester):
    source_slug = "new-york-ag"

    @retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=30))
    async def _fetch_page(self, client: httpx.AsyncClient, page: int = 0) -> str:
        # NY AG uses ?page=N pagination
    ...
    async def fetch_new_documents(self, since: datetime | None) -> list[RawDocumentCreate]:
        ...
        async with httpx.AsyncClient(
            ...
            while True:
                try:
                    ...
                    logger.error(f"New York AG: failed to fetch page {page}: {e}")
                    break
                ...
                # Check for pagination — NY AG uses Drupal pager
                next_link = soup.select_one("li.pager__item--next a, .pager-next a, .next a")
                if not next_link:
                    break
        ...
        logger.info(f"New York AG: collected {len(documents)} documents since {since.date()}")
        return documents
    ...
    def _parse_press_releases(
        ...
        """Parse press release items. Returns (docs, stop_early)."""
        documents: list[RawDocumentCreate] = []
        stop_early = False

        # NY AG uses Drupal views — common selectors
        items = (
        ...
        if not items:
            ...
            if content:
                items = content.find_all("li") or content.find_all("div")
...
# Celery task
ingester = NewYorkAGIngester()


@celery_app.task(
    name="workers.ingestion.new_york_ag.run",
    bind=True,
    max_retries=3,
...
def run(self):
    ...
    try:
        ...
        logger.error(f"New York AG ingestion failed: {exc}")
